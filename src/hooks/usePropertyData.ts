...
export const usePropertyData = () => {
  ...
  useEffect(() => {
    ...
    const generateSummary = async () => {
        ...
        setAiSummary(''); // Clear previous summary for streaming
...
${formatKpiForPrompt(previousYear, 'Last Year')}
        ...
        const prompt = `
            Analyze the following Singapore District 16 property data from ${filters.startDate} to ${filters.endDate}.
            Focus: ${fullFilterDescription || 'All property types'}.

            Overall Metrics:
            - Transactions: ${kpis.totalTransactions}
            - Volume: $${kpis.totalSalesVolume.toLocaleString()}
            - Avg. PSF: $${Math.round(kpis.averagePricePsf).toLocaleString()}
            // FIX: Corrected typo from `kpi` to `kpis`.
            - Avg. Profit: $${Math.round(kpis.averageProfit).toLocaleString()}
            - Highest Sale: $${kpis.highestTransaction.toLocaleString()}

            ${comparativeDataForPrompt}

            Instructions:
            1. Write a concise, insightful summary for a property investor. Start by stating the focus in bold (e.g., "For **Freehold properties**..."). Mention key trends.
            2. Create a markdown table comparing "Transaction Volume", "Average Price (PSF)", and "Average Profit" across these timeframes: "Current vs. Last Month", "Current vs. Last Quarter", and "Current YTD vs. Last Year". Show values as "Current vs. Previous". Use "N/A" for missing data.
            3. Format with markdown (bold, lists, table). Do not use markdown headers (e.g., ##).
        `;
        ...
        try {
            ...
            setAiSummaryError(`Failed to generate summary. Details: ${err.message}`);
        } finally {
